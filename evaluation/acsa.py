...
class DataModule(pl.LightningDataModule):
    ...
    def load_dataset(self):
        ...
        for mode in ('train', 'dev', 'test'):
            ...
            num_aspects = sum(len(set([(aspect['category'], aspect['polarity']) for aspect in example['aspects']]))
                              for example in self.raw_datasets[mode])

            print(f'{mode.upper():<5} | Sentences: {num_sentences:<5} | Aspect categories: {num_aspects:<5}')
...
class DataCollator:
    ...
    def make_labels(self, examples):
        ...
        for example in examples:
            target_seq = self.make_acsa_seq(example)
    ...
    def make_acsa_seq(self, example):
        if 'acsa_seq' in example:
            return example['acsa_seq']
        ...
        for aspect in example['aspects']:
            ...
            category_polarity = ' | '.join((category, polarity))
            if category_polarity not in acsa_seq:
                acsa_seq.append(category_polarity)

        acsa_seq = ' ; '.join(acsa_seq)

        example['acsa_seq'] = acsa_seq
        return acsa_seq
