...
echo "python atsa_5m23d.py fit: dataset ${dataset}, seed ${seed}"

CUDA_VISIBLE_DEVICES=${CUDA_IDS} python atsa_5m23d.py fit \
...
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python atsa_5m23d.py test \
  --seed_everything ${seed} \
  --trainer.devices=1 \
  --trainer.accelerator=gpu \
  --trainer.precision=${precision} \
